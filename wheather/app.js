    ...
    (req, res) => {
        ...
        MongoClient.connect('mongodb://localhost:27017/reforma', function (err, db) {
          if (err) throw err
            ...
            db.collection('clientReg').update({"_id":new ObjectId(req.body._id)},{$set:{"usrNm":req.body.usrNm,"femail":req.body.femail,"dob":req.body.dob,"phoneNo":req.body.phoneNo,"addr":req.body.addr}},function (err, result) {
                if (err) throw err
                else{
                    console.log(result)    
                    if (result) {                       
